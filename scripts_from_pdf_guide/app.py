...
# You can alternatively use the copied model:
#
# MODEL_PATH = (
#     PROJECT_ROOT
#     / "results"
#     / "wider_face_yolov8n_best.pt"
# )
...
# Use GPU 0 when available.
# Change this to "cpu" to force CPU inference.
DEVICE = 0
...
class FaceDetectionApp:
    ...
    def start_camera(self) -> None:
        """Open the webcam and begin processing frames."""
        if self.running:
            return
        ...
        # Request a common webcam resolution.
        self.camera.set(
    ...
    def update_frame(self) -> None:
        """Read, process and display the next webcam frame."""
        if not self.running:
            return
    ...
    def get_class_name(self, class_id: int) -> str:
        """Return the class name from the YOLO model."""
        if self.model is None:
            return "face"
    ...
    def resize_for_display(self, frame):
        """Resize the camera frame to fit inside the application window."""
        available_width = max(
